...
public sealed class SkillPage : ModInfoPage
{
    ...
    protected override void BuildContent(Transform root, float width)
    {
        ...
        AddText(hero, "Body",
            "修士将所悟功法化为法术。十二种元素、二十余种弹道、二十余种附魔词条相乘，世上没有两道完全相同的法术。金丹期可一气连发数十道，元婴以上则是铺天盖地的弹幕。",
        ...
        AddMiniCard(compose, "Elements", "cultiway/icons/iconTab", "十二种元素",
            "金·木·水·火·土·阴·阳·风·冰·雷·毒·混沌", width - 12f);
        AddBullet(compose, "弹道有直线、正弦波、螺旋追踪、回旋镖、抛物线、天降打击等二十余种。");
        AddBullet(compose, "附魔词条分四阶稀有度，从灼烧、冰冻到终焉死亡、永世诅咒。");
        AddBullet(compose, "混沌是游离于诸行之外的元素，不入灵根之列，唯道果与法术可驭。");
        ...
        AddText(poss, "Body",
            "附身一名修士，游戏瞬间变成 ARPG。长按施法键圈定目标、滚轮缩放范围、一键升空飞行——亲手把元婴期的千发弹幕砸向魔族大军。法宝与道果的主动神通，也一一列于指边。",
            6, FontStyle.Normal, TextAnchor.UpperLeft, PrimaryTextColor);
