...
internal sealed class BaibaoBlueprintInspector
{
    ...
    {
        GameObject root = UiElements.CreatePanel(parent, "ArtifactInspector", false, width, height, 3f,
            TextAnchor.UpperCenter);
        _preview = new BaibaoArtifactPreview(root.transform, width - 12f, height - 68f);

        GameObject primaryActions = UiLayout.Create(root.transform, "PrimaryActions", true,
            width - 8f, 22f, 3f, TextAnchor.MiddleCenter);
        _edit = CreateAction(primaryActions.transform, "Edit", UiIcons.Edit);
        _copy = CreateAction(primaryActions.transform, "Copy", UiIcons.Copy);
        _favorite = CreateAction(primaryActions.transform, "Favorite", UiIcons.Favorite);
        _gift = CreateAction(primaryActions.transform, "Gift", UiIcons.Gift);

        GameObject secondaryActions = UiLayout.Create(root.transform, "SecondaryActions", true,
            width - 8f, 22f, 3f, TextAnchor.MiddleCenter);
        _moveUp = CreateAction(secondaryActions.transform, "MoveUp", UiIcons.MoveUp, 90f);
        _moveDown = CreateAction(secondaryActions.transform, "MoveDown", UiIcons.MoveDown, 90f);
        _delete = CreateAction(secondaryActions.transform, "Delete", UiIcons.Delete);
    ...
    public void Show(ArtifactBlueprint blueprint, BaibaoBlueprintInspectorActions actions)
    {
        ...
        UiStateStyle.SetSelected(_favorite, actions.FavoriteSelected);
        UiStateStyle.SetSelected(_gift, actions.GiftSelected);
    ...
    {
        return UiElements.CreateIconButton(parent, name, icon, 34f, 21f, () => { }, 4f, 1f, rotation);
    }
