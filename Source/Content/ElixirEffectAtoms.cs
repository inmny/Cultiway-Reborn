...
public class ElixirEffectAtoms : ExtendLibrary<ElixirEffectAtomAsset, ElixirEffectAtoms>
{
    ...
    protected override void OnInit()
    {
        Set(Body, "body", ["淬体", "固元", "益血"], "血肉精气", "大幅增强体魄与血量",
            ...
            Stats((S.health, 5f)),
            r => 0.4f + Shape(r, ItemShapes.Blood, ItemShapes.Bone) + Element(r, ElementIndex.Earth));
        ...
        Set(HealthRegen, "health_regen", ["回春", "复元", "生肌"], "再生药性", "大幅提升生命恢复",
            ...
            r => Shape(r, ItemShapes.Blood, ItemShapes.Liquid, ItemShapes.Herb, ItemShapes.Root) + Element(r, ElementIndex.Wood, ElementIndex.Water));

        Set(WakanCapacity, "wakan_capacity", ["纳元", "聚灵", "广脉"], "灵海容量", "扩张灵力上限",
            Stats((BaseStatses.MaxWakan.id, 72f)),
            Stats((BaseStatses.MaxWakan.id, 8f)),
            r => Shape(r, ItemShapes.Crystal, ItemShapes.Stone, ItemShapes.Lotus, ItemShapes.Ball) + Element(r, ElementIndex.Water, ElementIndex.Pos, ElementIndex.Entropy));

        Set(WakanRegen, "wakan_regen", ["回元", "引灵", "归息"], "灵息回转", "提升灵力恢复",
            Stats((BaseStatses.WakanRegen.id, 4.2f)),
            Stats((BaseStatses.WakanRegen.id, 0.35f)),
            r => Shape(r, ItemShapes.Liquid, ItemShapes.Lotus, ItemShapes.Herb) + Element(r, ElementIndex.Water, ElementIndex.Wood, ElementIndex.Pos));

        Set(Mind, "mind", ["明心", "清神", "定识"], "清明神识", "大幅提升悟性",
            Stats((S.intelligence, 46f)),
            Stats((S.intelligence, 5f)),
            r => Shape(r, ItemShapes.Eye, ItemShapes.Fruit, ItemShapes.Lotus) + Element(r, ElementIndex.Pos, ElementIndex.Entropy),
            [ActorTraits.OpenSource.id]);

        Set(Life, "life", ["延寿", "驻年", "生息"], "寿元生机", "大幅延展寿元",
            Stats((S.multiplier_lifespan, 2.4f)),
            Stats((S.lifespan, 4f)),
            r => Shape(r, ItemShapes.Root, ItemShapes.Mushroom, ItemShapes.Fruit, ItemShapes.Wood) + Element(r, ElementIndex.Wood));

        Set(Speed, "speed", ["风行", "轻灵", "迅影"], "轻身药性", "大幅提升移动速度",
            Stats((S.multiplier_speed, 1.5f), (S.speed, 12f)),
            Stats((S.speed, 4f)),
            r => Shape(r, ItemShapes.Wing, ItemShapes.Feather, ItemShapes.Fur) + Element(r, ElementIndex.Neg));
        ...
        Set(AttackSpeed, "attack_speed", ["疾攻", "连锋", "急影"], "疾攻药性", "大幅提升攻击速度",
            ...
            r => Shape(r, ItemShapes.Claw, ItemShapes.Feather, ItemShapes.Wing, ItemShapes.Tooth) + Element(r, ElementIndex.Iron, ElementIndex.Fire, ElementIndex.Neg));

        Set(Damage, "damage", ["破锋", "摧锋", "裂刃"], "锋锐杀机", "大幅提升伤害",
            Stats((S.multiplier_damage, 1.4f), (S.damage, 16f)),
            Stats((S.damage, 3f)),
            r => Shape(r, ItemShapes.Claw, ItemShapes.Tooth, ItemShapes.Horn) + Element(r, ElementIndex.Iron, ElementIndex.Fire));

        Set(Crit, "crit", ["会心", "断机", "破绽"], "会心灵机", "大幅提升暴击",
            Stats((S.multiplier_crit, 1.1f), (S.critical_chance, 0.45f)),
            Stats((S.critical_chance, 0.035f)),
            r => Shape(r, ItemShapes.Eye, ItemShapes.Horn, ItemShapes.Tooth, ItemShapes.Crystal) + Element(r, ElementIndex.Iron, ElementIndex.Neg, ElementIndex.Entropy));

        Set(Accuracy, "accuracy", ["洞微", "命中", "照准"], "洞微感知", "大幅提升命中",
            Stats((S.accuracy, 70f)),
            Stats((S.accuracy, 4f)),
            r => Shape(r, ItemShapes.Eye, ItemShapes.Feather, ItemShapes.Horn) + Element(r, ElementIndex.Iron, ElementIndex.Pos, ElementIndex.Neg));

        Set(DamageRange, "damage_range", ["穿云", "裂甲", "洞金"], "贯穿锐意", "扩展伤害穿透范围",
            Stats((S.damage_range, 0.35f)),
            Stats((S.damage_range, 0.025f)),
            r => Shape(r, ItemShapes.Horn, ItemShapes.Tooth, ItemShapes.Stone, ItemShapes.Crystal) + Element(r, ElementIndex.Iron));

        Set(Armor, "armor", ["玄甲", "坚鳞", "护身"], "护体坚意", "大幅提升护甲",
            Stats((S.armor, 28f)),
            Stats((S.armor, 3f)),
            r => Shape(r, ItemShapes.Shell, ItemShapes.Stone, ItemShapes.Bone, ItemShapes.Hoof) + Element(r, ElementIndex.Iron, ElementIndex.Earth));
        ...
        Set(Soul, "soul", ["养魂", "凝魄", "神魂"], "魂魄清光", "壮大神魂",
            ...
            r => Shape(r, ItemShapes.Eye, ItemShapes.Crystal, ItemShapes.Ball, ItemShapes.Lotus) + Element(r, ElementIndex.Pos, ElementIndex.Entropy));
        ...
        Set(Qiyun, "qiyun", ["聚运", "天眷", "紫气"], "气运紫华", "汇聚气运",
            ...
            r => Shape(r, ItemShapes.Fruit, ItemShapes.Lotus, ItemShapes.Crystal, ItemShapes.Ball) + Quality(r, 1) + Element(r, ElementIndex.Pos));

        Set(ManaPool, "mana_pool", ["法力", "灵池", "玄泉"], "法力池泉", "大幅扩展法力",
            Stats((S.mana, 130f), (S.multiplier_mana, 2f)),
            Stats((S.mana, 8f)),
            r => Shape(r, ItemShapes.Liquid, ItemShapes.Crystal, ItemShapes.Lotus) + Element(r, ElementIndex.Water, ElementIndex.Pos, ElementIndex.Entropy));

        Set(StaminaPool, "stamina_pool", ["气力", "劲脉", "耐战"], "气力根基", "大幅提升气力",
            Stats((S.stamina, 120f), (S.multiplier_stamina, 1.8f)),
            Stats((S.stamina, 7f)),
            r => Shape(r, ItemShapes.Bone, ItemShapes.Hoof, ItemShapes.Root, ItemShapes.Shell) + Element(r, ElementIndex.Earth, ElementIndex.Wood));

        Set(CombatSkill, "combat_skill", ["战魄", "武胆", "斗罡"], "战技精魄", "大幅提升战技",
            Stats((S.skill_combat, 1.6f)),
            Stats((S.skill_combat, 0.07f)),
            r => Shape(r, ItemShapes.Claw, ItemShapes.Tooth, ItemShapes.Horn, ItemShapes.Bone, ItemShapes.Hoof) + Element(r, ElementIndex.Iron, ElementIndex.Fire, ElementIndex.Earth));

        Set(SpellSkill, "spell_skill", ["术心", "法印", "灵枢"], "术法灵枢", "大幅提升术法",
            Stats((S.skill_spell, 1.6f)),
            Stats((S.skill_spell, 0.07f)),
            r => Shape(r, ItemShapes.Eye, ItemShapes.Crystal, ItemShapes.Lotus, ItemShapes.Ball) + Element(r, ElementIndex.Water, ElementIndex.Pos, ElementIndex.Entropy));

        Set(Jindan, "jindan", ["丹核", "金丹", "抱元"], "金丹余韵", "温养金丹根基",
            Stats((BaseStatses.MaxWakan.id, 88f)),
            Stats((BaseStatses.MaxWakan.id, 7f)),
            r => Shape(r, ItemShapes.Ball) + HasJindan(r) + Quality(r, 1),
        ...
        Set(RootOpening, "root_opening", ["开灵", "启根", "通脉"], "灵根引子", "牵引灵根",
            ...
            r => Shape(r, ItemShapes.Root, ItemShapes.Lotus, ItemShapes.Crystal) + Quality(r, 1) + Element(r, ElementIndex.Wood, ElementIndex.Pos),
        ...
        Set(IronMastery, "iron_mastery", ["庚金", "金锋", "锐金"], "金行掌握", "大幅提升金行掌握",
            ...
            r => Element(r, ElementIndex.Iron) + Shape(r, ItemShapes.Stone, ItemShapes.Crystal, ItemShapes.Horn, ItemShapes.Tooth));
        ...
        Set(IronGuard, "iron_guard", ["金甲", "铁壁", "玄金"], "金行抗性", "大幅提升金行抗性",
            ...
            r => Element(r, ElementIndex.Iron) + Shape(r, ItemShapes.Shell, ItemShapes.Stone, ItemShapes.Bone));
        ...
        Set(WoodMastery, "wood_mastery", ["青木", "长青", "灵木"], "木行掌握", "大幅提升木行掌握",
            ...
            r => Element(r, ElementIndex.Wood) + Shape(r, ItemShapes.Root, ItemShapes.Wood, ItemShapes.Mushroom, ItemShapes.Fruit, ItemShapes.Herb));
        ...
        Set(WoodGuard, "wood_guard", ["木甲", "青屏", "生壁"], "木行抗性", "大幅提升木行抗性",
            ...
            r => Element(r, ElementIndex.Wood) + Shape(r, ItemShapes.Root, ItemShapes.Wood, ItemShapes.Shell));
        ...
        Set(WaterMastery, "water_mastery", ["玄水", "寒泉", "澄流"], "水行掌握", "大幅提升水行掌握",
            ...
            r => Element(r, ElementIndex.Water) + Shape(r, ItemShapes.Liquid, ItemShapes.Lotus, ItemShapes.Crystal));
        ...
        Set(WaterGuard, "water_guard", ["水幕", "寒甲", "玄流"], "水行抗性", "大幅提升水行抗性",
            ...
            r => Element(r, ElementIndex.Water) + Shape(r, ItemShapes.Liquid, ItemShapes.Lotus, ItemShapes.Shell));
        ...
        Set(FireMastery, "fire_mastery", ["离火", "炎华", "赤炼"], "火行掌握", "大幅提升火行掌握",
            ...
            r => Element(r, ElementIndex.Fire) + Shape(r, ItemShapes.Blood, ItemShapes.Crystal, ItemShapes.Tooth, ItemShapes.Horn));
        ...
        Set(FireGuard, "fire_guard", ["火衣", "炎甲", "赤障"], "火行抗性", "大幅提升火行抗性",
            ...
            r => Element(r, ElementIndex.Fire) + Shape(r, ItemShapes.Shell, ItemShapes.Stone, ItemShapes.Blood));
        ...
        Set(EarthMastery, "earth_mastery", ["坤土", "厚岳", "山魄"], "土行掌握", "大幅提升土行掌握",
            ...
            r => Element(r, ElementIndex.Earth) + Shape(r, ItemShapes.Stone, ItemShapes.Bone, ItemShapes.Root, ItemShapes.Shell));
        ...
        Set(EarthGuard, "earth_guard", ["土甲", "山壁", "镇岳"], "土行抗性", "大幅提升土行抗性",
            ...
            r => Element(r, ElementIndex.Earth) + Shape(r, ItemShapes.Stone, ItemShapes.Bone, ItemShapes.Shell));
        ...
        Set(YinMastery, "yin_mastery", ["玄阴", "幽冥", "阴华"], "阴性掌握", "大幅提升阴性掌握",
            ...
            r => Element(r, ElementIndex.Neg) + Shape(r, ItemShapes.Fur, ItemShapes.Wing, ItemShapes.Silk, ItemShapes.Eye));
        ...
        Set(YinGuard, "yin_guard", ["阴甲", "幽障", "玄屏"], "阴性抗性", "大幅提升阴性抗性",
            ...
            r => Element(r, ElementIndex.Neg) + Shape(r, ItemShapes.Fur, ItemShapes.Shell, ItemShapes.Silk));
        ...
        Set(YangMastery, "yang_mastery", ["阳华", "曜光", "明阳"], "阳性掌握", "大幅提升阳性掌握",
            ...
            r => Element(r, ElementIndex.Pos) + Shape(r, ItemShapes.Lotus, ItemShapes.Eye, ItemShapes.Crystal, ItemShapes.Fruit));
        ...
        Set(YangGuard, "yang_guard", ["阳甲", "明障", "曜屏"], "阳性抗性", "大幅提升阳性抗性",
            ...
            r => Element(r, ElementIndex.Pos) + Shape(r, ItemShapes.Lotus, ItemShapes.Shell, ItemShapes.Crystal));
        ...
        Set(EntropyMastery, "entropy_mastery", ["混沌", "浊玄", "归墟"], "混沌掌握", "大幅提升混沌掌握",
            ...
            r => Element(r, ElementIndex.Entropy) + Shape(r, ItemShapes.Crystal, ItemShapes.Eye, ItemShapes.Ball, ItemShapes.Stone));
        ...
        Set(EntropyGuard, "entropy_guard", ["混甲", "浊障", "归墟"], "混沌抗性", "大幅提升混沌抗性",
            ...
            r => Element(r, ElementIndex.Entropy) + Shape(r, ItemShapes.Crystal, ItemShapes.Shell, ItemShapes.Stone));
    }
    ...
    private static float Shape(ElixirRecipeContext recipe, params ItemShapeAsset[] shapes)
    {
        if (string.IsNullOrEmpty(recipe.main_shape_id) || shapes == null) return 0f;
        for (var i = 0; i < shapes.Length; i++)
        {
            var shape = shapes[i];
            if (shape != null && recipe.main_shape_id == shape.id) return 4f;
        }
        return 0f;
    }
