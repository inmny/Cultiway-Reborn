...
/// <summary>按少量地块逐步把浊气向邻格摊开。</summary>
public class DirtyWakanSpreadSystem : BaseSystem, IWorldStateClearable
{
    ...
    {
        if (MapGenerator._tilesMap != null || World.world?.tiles_list == null ||
            !WorldWakanService.IsInitialized)
        {
            return;
        }
    ...
    private void CheckSingleTile(WorldTile tile)
    {
        ...
        foreach (WorldTile neighbor in tile.neighbours)
        {
            ...
            float tileValue = WorldWakanService.GetDirty(tileId);
            float neighborValue = WorldWakanService.GetDirty(neighbor.data.tile_id);
            ...
            float flow = Mathf.Sign(delta) * Mathf.Abs(delta) *
                Mathf.Clamp(Mathf.Log10(Mathf.Max(1f, totalTileCount / (float)CheckTileCountPerFrame)) * 0.1f,
                    0f, 1f);
            if (flow > 0f)
                WorldWakanService.TransferDirty(tileId, neighbor.data.tile_id, flow);
            else if (flow < 0f)
                WorldWakanService.TransferDirty(neighbor.data.tile_id, tileId, -flow);
        }
    }
