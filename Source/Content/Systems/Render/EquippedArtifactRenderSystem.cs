...
/// </summary>
public class EquippedArtifactRenderSystem : QuerySystem<ActorBinder, ArtifactLoadoutState>
{
    ...
    protected override void OnUpdate()
    {
        ...
        Query.ForEachEntity([Hotfixable](ref ActorBinder binder, ref ArtifactLoadoutState _, Entity owner) =>
        {
            ...
            {
                if (relations[i].artifact.IsAvailable()) artifactCount++;
            }
            ...
            for (int i = 0; i < relations.Length; i++)
            {
                ...
                renderer.sprite_renderer.color = relation.state.GetStateColor(time);
                renderer.sprite_renderer.flipX = false;
                renderer.sprite_renderer.sortingOrder = 6 + (int)relation.state;
                renderer.transform.localRotation = Quaternion.identity;
                renderer.transform.localScale = Vector3.one *
                                                ResolveScale(sprite, actorScale, time, visualIndex) *
                                                relation.state.GetStateScale();
