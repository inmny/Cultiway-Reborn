...
internal static class PatchActor
{
    ...
    {/*
        ...
        if (CanUseCultiwayJobSelection(pActor))
        {
            ...
            var use_cultivator_job = Randy.randomChance(pActor.hasTrait(ActorTraits.Cultivator.id) ? 0.8f : 0.2f);

            // 魔法事务只在本轮主动选中时覆盖原版工作；双体系单位未选中时继续参与仙道工作选择。
            if (ae.HasCultisys<Magic>())
            {
                if (use_cultivator_job) 
                {
                    pool.Add(ActorJobs.MagicCultivator.id);
                }
                else
                {
