...
public static class CraftFailureService
{
    ...
    public static bool Fail(Entity craftingItem, CraftFailureReason reason)
    {
        ...
        return FailCore(craftingItem, process, reason, originalName);
    }

    public static bool Fail(Entity craftingItem, CraftProcessType process, CraftFailureReason reason)
    {
        if (craftingItem.IsNull || craftingItem.HasComponent<CraftWaste>()) return false;
        string originalName = ResolveOriginalName(craftingItem, process);
        return FailCore(craftingItem, process, reason, originalName);
    }

    private static bool FailCore(Entity craftingItem, CraftProcessType process,
        CraftFailureReason reason, string originalName)
    {
        if (craftingItem.IsNull || craftingItem.HasComponent<CraftWaste>()) return false;
        var ingredients = craftingItem.GetRelations<CraftOccupyingRelation>();
        var ingredientEntities = new Entity[ingredients.Length];
        for (int i = 0; i < ingredients.Length; i++) ingredientEntities[i] = ingredients[i].item;
        ...
        if (craftingItem.HasComponent<Elixir>()) craftingItem.RemoveComponent<Elixir>();
        if (craftingItem.HasComponent<Artifact>()) craftingItem.RemoveComponent<Artifact>();
        craftingItem.RemoveTags(Tags.Get<TagElixirRestore, TagElixirStatusGain, TagElixirDataGain>());
    ...
    private static bool TryResolveProcess(Entity craftingItem,
        out CraftProcessType process, out string originalName)
    {
        process = default;
        originalName = null;
        if (craftingItem.IsNull || craftingItem.HasComponent<CraftWaste>()) return false;
        if (craftingItem.TryGetComponent(out CraftingElixir _))
            process = CraftProcessType.Alchemy;
        else if (craftingItem.HasComponent<CraftingArtifact>())
            process = CraftProcessType.ArtifactRefining;
        else if (craftingItem.TryGetComponent(out CraftSession session))
            process = session.process;
        else
            return false;
        originalName = ResolveOriginalName(craftingItem, process);
        return true;
    }

    private static string ResolveOriginalName(Entity craftingItem, CraftProcessType process)
    {
        if (process == CraftProcessType.Alchemy &&
            craftingItem.TryGetComponent(out CraftingElixir craftingElixir))
        {
            ...
            if (asset != null) return asset.GetName();
        }
        return craftingItem.HasName ? craftingItem.Name.value : null;
    }
